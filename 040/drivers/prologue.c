...
int main() {
    ...
    hp--; // hp points to prologue footer
    if (*hp != 17) {
        printf("[ERROR]: Prologue footer %p set to %lu but should be %d!\n",
                hp, (size_t)*hp, 17);
        exit(1);
    }
    hp = (Header *)(chp - MIN_BLOCK_SIZE); // hp points to prologue header
    if (*hp != 17) {
        printf("[ERROR]: Prologue header %p set to %lu but should be %d!\n",
                hp, (size_t)*hp, 17);
        exit(1);
    }
    free(heap);
