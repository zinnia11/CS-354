...
int main() {
    ...
    if (!p) {
        ...
        printf("[ERROR]: Returned null on 15 byte allocation request in 1600 byte heap!\n");
        exit(1);
    }
    p[ 0] = 't';
    p[ 1] = 'h';
    ...
    if (p != chp) {
        printf("[INFO ]: Heap start is at address %p\n", chp);
        printf("[ERROR]: Must use a first fit policy!\n");
        exit(1);
    }
    if ((unsigned long)p % (unsigned long)16) {
        printf("[INFO ]: Heap start is at address %p\n", chp);
        printf("[ERROR]: Returned non-aligned pointer\n");
        exit(1);
    }
    ...
    if ((unsigned long)(*hp) != 33) {
        printf("[INFO ]: Heap start is at address %p\n", chp);
        printf("[ERROR]: Allocated block must be 32 bytes and have LSB set!\n");
        exit(1);
    }
    chp = chp + ((unsigned long)(*hp)&(~0x1));
    hp = (Header *) chp;
    hp--;
    // hp now points to the header of the split block
    Header block_size = HEAP_SIZE - 32;
    block_size &= (~0x1); // ignore LSB at first to just check size
